// Script de test pour le système d'humeur actuel
// Exécutez ce script dans la console du navigateur pour tester

console.log('🧪 Test du système d\'humeur actuel...');

async function testCurrentMoodSystem() {
  try {
    console.log('🚀 Test du système d\'humeur actuel...');
    ...
    if (typeof window.supabase === 'undefined') {
      ...
      return false;
         ...
         // Test 1: Vérifier la structure actuelle de la table profiles
     console.log('\n📊 Test 1: Vérification de la structure de la table profiles');
     try {
       const { data: tableCheck, error: tableError } = await window.supabase
         .from('profiles')
         .select('id, name, birth_date, gender, country, interests, profile_picture, mood, created_at')
         .limit(1);
      ...
      if (tableError) {
        ...
        console.log('   🎯 Champ mood présent:', 'mood' in (tableCheck?.[0] || {}));
      }
    } catch (error) {
    ...
    const { data: profile, error: profileError } = await window.supabase
      .from('profiles')
      .select('id, name, profile_picture, mood')
      .eq('id', user.id)
      .single();
    ...
    console.log('   • Photo de profil:', profile.profile_picture ? 'Oui' : 'Non');
    ...
    if (coupleProfiles && coupleProfiles.length > 0) {
      coupleProfiles.forEach((profile, index) => {
        console.log(`     ${index + 1}. ${profile.name || 'Utilisateur'}: ${profile.mood || 'neutre'} (Photo: ${profile.profile_picture ? 'Oui' : 'Non'})`);
      });
    }
    
    // Test 6: Tester la mise à jour d'une humeur
    console.log('\n➕ Test 6: Test de mise à jour d\'une humeur');
    const testMood = 'joyeux';
    ...
    console.log('\n🎯 Test du système d\'humeur actuel terminé avec succès!');
    return true;
...
async function createTestMood(moodType = 'content') {
  try {
...
window.testCurrentMoodSystem = testCurrentMoodSystem;
window.createTestMood = createTestMood;
window.showCoupleProfiles = showCoupleProfiles;

console.log('\n✅ Script de test du système d\'humeur actuel chargé!');
console.log('Fonctions disponibles:');
console.log('  • testCurrentMoodSystem() - Test complet du système d\'humeur actuel');
...
console.log('\n💡 Commencez par: testCurrentMoodSystem()');
